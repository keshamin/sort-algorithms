class BinTree(object):
    ...
    def __repr__(self):
        ...
        return '<Node, value: {}, left: {}, right: {}>'.format(self.value, left_value, right_value)
    ...
    def add_child(self, child_value):
        if self.value > child_value:
            if self.left is None:
                self.left = BinTree(child_value)
            elif isinstance(self.left, BinTree):
                self.left.add_child(child_value)
            else:
                raise ValueError
        else:
            if self.right is None:
                self.right = BinTree(child_value)
            elif isinstance(self.right, BinTree):
